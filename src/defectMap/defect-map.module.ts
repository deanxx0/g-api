...
import { InferenceResult, InferenceResultSchema } from 'src/inferenceResult/schema/inference-result.schema';
import { DefectMapController } from './defect-map.controller';
import { DefectMapService } from './defect-map.service';

@Module({
  imports: [
    MongooseModule.forFeature([{ name: InferenceResult.name, schema: InferenceResultSchema }]),
  ],
