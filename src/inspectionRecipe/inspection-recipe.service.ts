...
import {
  InspectionRecipe,
  InspectionRecipeDocument,
} from './schema/inspection-recipe.schema';

@Injectable()
export class InspectionRecipeService {
  constructor(
    @InjectModel(InspectionRecipe.name)
    private inspectionRecipeModel: Model<InspectionRecipeDocument>,
  ) {}

  async create(
    createInspectionRecipeDto: CreateInspectionRecipeDto,
  ): Promise<InspectionRecipeDocument> {
    const createdSensor = new this.inspectionRecipeModel(
